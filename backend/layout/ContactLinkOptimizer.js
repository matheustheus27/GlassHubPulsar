...
class ContactLinkOptimizer {
  ...
  static getSvgIcon(item) {
    ...
    const icon = String(item.icon || "").toLowerCase().trim();
    ...
    if (item.customIcon || item.iconUrl || item.favicon || icon.startsWith('data:')) {
    ...
    // 1. Email
    if (link.includes("mailto") || title.includes("@") || icon === "email" || icon === "✉️" || icon === "mail") {
    ...
    // 2. Phone / WhatsApp
    if (link.includes("wa.me") || link.includes("phone") || link.includes("tel") || icon === "phone" || icon === "📞" || icon === "whatsapp") {
    ...
    // 5. Portfolio / Website (Prioritized for explicit portfolio items, github.io sites, or custom domains)
    const isExplicitPortfolio = icon === "portfolio" || icon === "portfólio" || icon === "🌐" || icon === "globe" || icon === "website" || icon === "site" || title.includes("portfólio") || title.includes("portfolio") || link.includes("github.io");
    ...
    // 6. GitHub (Strictly for actual GitHub profiles/repositories, not portfolio websites)
    if ((link.includes("github.com") || title.includes("github") || icon === "github" || icon === "🐙") && !isExplicitPortfolio) {
    ...
    // 9. Facebook
    if (link.includes("facebook") || link.includes("fb.com") || title.includes("facebook") || icon === "facebook" || icon === "📘") {
    ...
    // 10. Generic Website / Link Fallback
    if (link.includes("portfolio") || title.includes("portfólio") || title.includes("portfolio") || icon === "portfolio" || icon === "🌐" || link.startsWith("http")) {
      return `<svg class="svg-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>`;
    }
