...
async function runTests() {
  ...
  const noisyPdfLeak = `
PDF-1.4
/Title (MATHEUS THIAGO DE SOUZA FERREIRA)
...
📍 Ribeirão das Neves, MG ✉️ matheustheus27@gmail.com 📞 +55 (31) 99150-4604 🐙 GitHub 💼 LinkedIn
`;
  ...
  assert(cleaned.includes('MATHEUS THIAGO DE SOUZA FERREIRA'), 'Should preserve candidate name');
...
Commit Jr. (Empresa Júnior de Engenharia de Computação do CEFET-MG) Fev 2021 – Fev 2023
...
Núcleo de Tecnologia da Informação e Comunicação (NTIC - CEFET-MG) Nov 2019 – Nov 2020
Especialista de TI
...
Sistema Divina Providência Mar 2017 – Fev 2019
...
FORMAÇÃO ACADÊMICA
CEFET-MG Previsão de conclusão em 2028
...
Sistema Divina Providência Concluído em 2015
...
`;
  ...
  assert.strictEqual(parsed.personalDetails.name, 'MATHEUS THIAGO DE SOUZA FERREIRA', 'Candidate name must be accurate');
  assert.strictEqual(parsed.personalDetails.title, 'DESENVOLVEDOR DE SOFTWARE', 'Candidate title must be accurate');
  assert.strictEqual(parsed.personalDetails.contact.email.email, 'matheustheus27@gmail.com', 'Email must match');
  assert(parsed.personalDetails.contact.phone.phone.includes('99150-4604'), 'Phone number must match');
  assert(parsed.personalDetails.location.location.includes('Ribeirão das Neves'), 'Location must be detected');
  ...
  assert(semanticHtml.includes('<h1 class="candidate-name">MATHEUS THIAGO DE SOUZA FERREIRA</h1>'), 'Must generate candidate name header');
