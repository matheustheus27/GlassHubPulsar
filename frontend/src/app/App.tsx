...
export default function App() {
  ...
  const styles: Settings = settingsData[theme];
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      setToast({
        message: locale === 'pt-BR' ? 'PDF exportado com sucesso!' : 'PDF exported successfully!',
        type: 'success'
      });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      setToast({
        message: locale === 'pt-BR' ? `Erro ao exportar: ${errorMessage}` : `Export error: ${errorMessage}`,
        type: 'error'
      });
  ...
  // Prepares current active document payload for the AI Chat context
  const currentPayload = activeTab === 'resume' 
    ? buildResumePayload(locale, isLight, styles) 
    : buildCoverPayload(locale, isLight, styles);
  ...
  return (
    <div 
      className="min-h-screen flex justify-center p-6 md:p-12 font-sans transition-colors duration-300 relative"
    ...
    >
      <div className="w-full max-w-3xl">
        ...
        <div className="flex justify-between items-center mb-6 print:hidden">
          <div className="flex gap-2">
            ...
            >
              ⚙️ {locale === 'pt-BR' ? 'Configurações' : 'Settings'}
            </button>

            <button
              onClick={() => setOpenAIChat(!openAIChat)}
              className="px-3 py-2 rounded-lg border text-xs font-medium hover:bg-cyan-800 hover:text-white transition cursor-pointer text-cyan-400 border-slate-800"
            >
              ✨ {locale === 'pt-BR' ? 'Analisar com IA' : 'Analyze with AI'}
            </button>
          </div>
          ...
          >
            {isExporting 
              ? (locale === 'pt-BR' ? 'Exportando...' : 'Exporting...') 
              : (locale === 'pt-BR' ? 'Exportar PDF' : 'Export PDF')}
          </button>
        </div>
        ...
        <div className="flex gap-2 mb-6 border-b border-slate-800 pb-2 print:hidden">
          ...
          >
            {locale === 'pt-BR' ? 'Carta de Apresentação' : 'Cover Letter'}
          </button>
        </div>
      ...
      <AIChatDrawer
        ...
        documentPayload={currentPayload}
        lang={locale}
        style={{ backgroundColor: styles.card.backgroundColor, borderColor: styles.card.borderColor }}
      />
    </div>
